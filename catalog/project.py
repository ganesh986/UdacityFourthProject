...
def gconnect():
    ...
    if stored_access_token is not None and gplus_id == stored_gplus_id:
        response = make_response(json.dumps(
            'Current useris already connected.'), 200)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def showCategories():
    ...
    lastItems = session.query(CategoryItem).order_by(desc(
        CategoryItem.insert_date))
    if 'username' not in login_session:
        return render_template(
            'publicmenu.html', categories=categories, lastItems=lastItems)
    else:
        return render_template(
            'menu.html',  categories=categories, lastItems=lastItems)
...
def showItem(category_id, item_id):
    ...
    item = session.query(CategoryItem).filter_by(
        categories_id=category_id, id=item_id).one()
    if 'username' not in login_session:
        return render_template(
            'pubblicitem.html',
            item=item, category=category, categories=categories)
    else:
        return render_template(
            'item.html',
            item=item, category=category, categories=categories)
...
@app.route('/<int:category_id>/newitem/', methods=['GET', 'POST'])
def newCategoryItem(category_id):
    ...
    if request.method == 'POST':
        newItem = CategoryItem(
            ...
            price=request.form['price'], insert_date=datetime.datetime.now(),
            categories_id=category.id)
        session.add(newItem)
        session.commit()
        flash('New Item Successfully Created')
        return redirect(url_for(
            'showCategoryItems', category_id=category_id,
            categories=categories))
    else:
        return render_template(
...
def editItem(category_id, item_id):
    ...
    if item.user_id != login_session['user_id']:
        return "<script>function myFunction() "\
            ...
            "your own category in order to edit.');}</script>"\
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for(
            'showItem', category_id=category_id, item_id=item_id, item=item))
    else:
        return render_template(
            'edititem.html',
            category_id=category_id, category=category,  item=item)
...
def deleteItem(category_id, item_id):
    ...
    if itemToDelete.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are "\
            "not authorized to delete this item. Please create your "\
            "own category in order to delete.');}</script>"\
            "<body onload='myFunction()'>"
    category = session.query(Categories).filter_by(id=category_id).one()
    if request.method == 'POST':
        ...
        return render_template(
            ...
            category=category, item=itemToDelete)
